...
export default function ManagerLogin() {
  ...
  const { loginManager, authLoading } = useAuth()

  const [form, setForm] = useState({ email: '', password: '', code: '', recaptchaToken: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await loginManager(form)
  ...
  return (
    <div className="rounded-[2rem] border border-white/10 bg-white/5 backdrop-blur-xl p-8 md:p-10 shadow-[0_30px_100px_rgba(0,0,0,0.45)]">
      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-emerald-400/20 bg-emerald-400/10 text-emerald-300 text-sm">
        <ShieldCheck className="w-4 h-4" />
        <span>Manager Portal</span>
      </div>

      <h1 className="mt-6 text-3xl font-semibold">Sign in to the management workspace</h1>
      <p className="mt-3 text-gray-400">
        Human verification and two-factor codes are enforced for manager accounts.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
        ...
        >
          <span>{authLoading ? 'Signing in...' : 'Login as Manager'}</span>
