...
import { Activity, BrainCircuit, Clock3, Eye, Mic, ScanFace, Search, ShieldAlert, Trash2 } from 'lucide-react'
            ...
            <>
              ...
              <div className="rounded-xl border border-gray-700 bg-black/20 p-4">
                <div className="flex items-center gap-2 text-gray-300">
                  ...
                  <p className="font-semibold text-white">Professional interview signals</p>
                </div>
                <div className="mt-4 grid md:grid-cols-2 xl:grid-cols-4 gap-3">
                  {[
                    { key: 'communication', label: 'Communication', icon: Mic, payload: selected.behavioralSignals?.communication },
                    { key: 'posture', label: 'Posture', icon: ScanFace, payload: selected.behavioralSignals?.posture },
                    { key: 'eyeMovement', label: 'Eye Movement', icon: Eye, payload: selected.behavioralSignals?.eyeMovement },
                    { key: 'movement', label: 'Movement', icon: Activity, payload: selected.behavioralSignals?.movement },
                  ].map((item) => {
                    const Icon = item.icon
                    const score = Number(item.payload?.score || 0)
                    return (
                      <div key={item.key} className="rounded-xl border border-gray-700 bg-gray-900/60 p-3">
                        <div className="flex items-center gap-2 text-gray-400">
                          <Icon className="w-4 h-4" />
                          <span className="text-xs uppercase tracking-[0.16em]">{item.label}</span>
                        </div>
                        <p className="mt-2 text-xl font-semibold text-white">{score ? `${score.toFixed(1)}/5` : 'Pending'}</p>
                        <p className="mt-1 text-sm text-gray-300">{item.payload?.confidence || item.payload?.label || 'No signal yet'}</p>
                        <p className="mt-2 text-xs text-gray-500">{item.payload?.summary || 'No summary available yet.'}</p>
                      </div>
                    )
                  })}
                </div>
                <div className="mt-4 rounded-xl border border-gray-700 bg-gray-900/60 p-3">
                  <p className="text-xs uppercase tracking-[0.16em] text-gray-500">Integrity risk</p>
                  <p className={`mt-2 text-lg font-semibold ${
                    selected.integritySignals?.cheatingRisk === 'Low'
                      ? 'text-emerald-300'
                      : selected.integritySignals?.cheatingRisk === 'Medium'
                        ? 'text-amber-300'
                        : 'text-red-300'
                  }`}>
                    {selected.integritySignals?.cheatingRisk || 'Pending'}
                  </p>
                  <p className="mt-2 text-sm text-gray-400">{selected.integritySignals?.summary || 'No integrity summary captured.'}</p>
                </div>
              </div>
