...
class AssessmentRecord(Base):
    ...
    behavioral_signals_json: Mapped[str] = mapped_column(Text, default="{}", nullable=False)
    integrity_signals_json: Mapped[str] = mapped_column(Text, default="{}", nullable=False)
...
def init_database() -> None:
    ...
    ensure_assessment_behavior_columns()
...
def ensure_assessment_behavior_columns() -> None:
    try:
        with engine.begin() as connection:
            result = connection.execute(
                text(
            ...
            columns = {row[0] for row in result}
            if "behavioral_signals_json" not in columns:
                connection.execute(text("ALTER TABLE assessments ADD COLUMN behavioral_signals_json TEXT NOT NULL DEFAULT '{}'"))
            if "integrity_signals_json" not in columns:
                connection.execute(text("ALTER TABLE assessments ADD COLUMN integrity_signals_json TEXT NOT NULL DEFAULT '{}'"))
    except SQLAlchemyError:
        return
