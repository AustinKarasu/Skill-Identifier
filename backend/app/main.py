...
AUTH_LOCKOUTS: dict[str, dict[str, Any]] = {}
AUTH_RATE_LIMIT_WINDOW_SECONDS = 60 * 10
AUTH_RATE_LIMIT_MAX_ATTEMPTS = 8
AUTH_LOCKOUT_THRESHOLD = 5
AUTH_LOCKOUT_SECONDS = 60 * 15
...
def build_session(user: UserRecord) -> dict[str, Any]:
    ...
    ACTIVE_SESSIONS[token] = {
        "userId": user.id,
        "createdAt": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(now_ts)),
        "expiresAtTs": now_ts + SESSION_COOKIE_MAX_AGE,
        "device": "Web browser",
        "csrfToken": csrf_token,
    }
    return {"token": token, "user": serialize_user(user), "csrfToken": csrf_token}
...
def cleanup_active_sessions() -> None:
    ...
    stale_tokens = [
        ...
        if not details.get("userId") or float(details.get("expiresAtTs", 0) or 0) <= now
    ]
    for token in stale_tokens:
        ACTIVE_SESSIONS.pop(token, None)


def cleanup_auth_rate_limits() -> None:
    now = time.time()
    cutoff = now - AUTH_RATE_LIMIT_WINDOW_SECONDS
    for key in list(AUTH_RATE_LIMITS.keys()):
        recent = [attempt for attempt in AUTH_RATE_LIMITS[key] if attempt > cutoff]
        if recent:
            AUTH_RATE_LIMITS[key] = recent
        else:
            AUTH_RATE_LIMITS.pop(key, None)


def cleanup_auth_lockouts() -> None:
    now = time.time()
    for key in list(AUTH_LOCKOUTS.keys()):
        locked_until = float(AUTH_LOCKOUTS[key].get("lockedUntilTs", 0) or 0)
        if locked_until and locked_until > now:
            continue
        if int(AUTH_LOCKOUTS[key].get("failures", 0) or 0) <= 0:
            AUTH_LOCKOUTS.pop(key, None)
        else:
            AUTH_LOCKOUTS[key]["lockedUntilTs"] = 0


def build_auth_rate_limit_key(action: str, request: Request, identity: str = "") -> str:
    client_host = request.client.host if request and request.client else "unknown"
    return f"{action}:{client_host}:{str(identity or '').strip().lower()}"


def enforce_auth_rate_limit(action: str, request: Request, identity: str = "") -> str:
    cleanup_auth_rate_limits()
    key = build_auth_rate_limit_key(action, request, identity)
    attempts = AUTH_RATE_LIMITS.setdefault(key, [])
    if len(attempts) >= AUTH_RATE_LIMIT_MAX_ATTEMPTS:
        raise HTTPException(status_code=429, detail="Too many attempts. Please wait a few minutes before trying again.")
    attempts.append(time.time())
    return key


def clear_auth_rate_limit(key: str | None) -> None:
    if key:
        AUTH_RATE_LIMITS.pop(key, None)


def enforce_auth_lockout(action: str, request: Request, identity: str = "") -> str:
    cleanup_auth_lockouts()
    key = build_auth_rate_limit_key(action, request, identity)
    record = AUTH_LOCKOUTS.get(key, {})
    locked_until = float(record.get("lockedUntilTs", 0) or 0)
    if locked_until > time.time():
        raise HTTPException(status_code=423, detail="Account temporarily locked due to repeated failed attempts. Please try again later.")
    return key


def clear_auth_lockout(key: str | None) -> None:
    if key:
        AUTH_LOCKOUTS.pop(key, None)


def notify_managers_of_security_alert(session, title: str, message: str) -> None:
    managers = session.scalars(select(UserRecord).where(UserRecord.role == "manager")).all()
    for manager in managers:
        create_inapp_notification(session, manager, title, message, "security")
    recipients = get_manager_recipients(session, "security")
    if recipients:
        send_email_async(title, message, recipients)
...
def register_auth_failure(
    ...
    *,
    ...
    key = build_auth_rate_limit_key(action, request, identity)
    record = AUTH_LOCKOUTS.setdefault(key, {"failures": 0, "lockedUntilTs": 0})
    failures = int(record.get("failures", 0) or 0) + 1
    record["failures"] = failures
    lockout_triggered = failures >= AUTH_LOCKOUT_THRESHOLD
    if lockout_triggered:
        record["lockedUntilTs"] = time.time() + AUTH_LOCKOUT_SECONDS
        alert_message = (
            f"Repeated failed {action.replace('_', ' ')} attempts detected for {target or 'unknown identity'} "
            f"from {request.client.host if request and request.client else 'unknown host'}. "
            f"Temporary lockout applied for {AUTH_LOCKOUT_SECONDS // 60} minutes."
        )
        notify_managers_of_security_alert(session, "Security alert: auth lockout", alert_message)
    append_audit_log(
        session,
        actor_id=None,
        ...
        meta={
            "failures": failures,
            "lockedOut": lockout_triggered,
            "clientHost": request.client.host if request and request.client else "unknown",
        },
    )


def verify_user_password(user: UserRecord, candidate_password: str) -> bool:
    return verify_password(candidate_password, str(user.password_hash or ""))


def maybe_upgrade_password_hash(user: UserRecord, plain_password: str) -> bool:
    stored_hash = str(user.password_hash or "")
    if not is_legacy_password_hash(stored_hash):
        return False
...
def validate_csrf(request: Request) -> None:
    ...
    if not session_token:
        return
    session_state = ACTIVE_SESSIONS.get(session_token, {})
    expected_token = str(session_state.get("csrfToken") or "").strip()
    cookie_token = str(request.cookies.get(CSRF_COOKIE_NAME) or "").strip()
    header_token = str(request.headers.get("X-CSRF-Token") or "").strip()
    if not expected_token or not cookie_token or not header_token:
        raise HTTPException(status_code=403, detail="Missing CSRF protection token")
    if not hmac.compare_digest(expected_token, cookie_token) or not hmac.compare_digest(expected_token, header_token):
        raise HTTPException(status_code=403, detail="Invalid CSRF protection token")
...
def update_settings_for_user(session, user: UserRecord, section: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if section == "profile" and "photoData" in normalized:
        normalized["photoData"] = validate_profile_photo_data(normalized.get("photoData"))
...
def profile_completion_status(profile: dict[str, Any] | None, *, current_status: str = "pending") -> str:
    ...
    completed = sum(1 for item in checkpoints if item)
    if completed >= 4:
        return "active"
    if completed >= 2:
        return "in-progress"
...
def sync_employee_record_from_profile(
    session,
    employee_id: str,
    ...
    *,
    ...
    payload = profile if isinstance(profile, dict) else {}
    normalized_email = normalize_email(payload.get("email") or fallback_email)
    if not normalized_email and not str(employee_id or "").strip():
        return None

    employee = None
    if normalized_email:
        employee = session.scalar(select(EmployeeRecord).where(EmployeeRecord.email == normalized_email))
    if not employee:
        employee_users = session.scalars(select(UserRecord).where(UserRecord.role == "employee")).all()
        for item in employee_users:
            if str(item.id or "").strip() != str(employee_id or "").strip():
                continue
            employee = session.scalar(select(EmployeeRecord).where(EmployeeRecord.email == item.email))
            if employee:
                break

    full_name = str(payload.get("fullName") or fallback_name or "").strip()
    role = str(payload.get("role") or "").strip()
    department = str(payload.get("department") or fallback_department or "").strip()
    ...
    if employee:
        ...
        if role:
            employee.role = role
        employee.status = profile_completion_status(payload, current_status=employee.status)
        return employee
    ...
    employee = EmployeeRecord(
        name=full_name or derive_name_from_email(normalized_email),
        email=normalized_email,
        role=role or f"{department or 'General'} Specialist",
        skills_json=dumps_json([]),
        skill_level=0.0,
        status=profile_completion_status(payload, current_status="pending"),
...
def decode_base64_data_url(payload: str) -> tuple[str, bytes]:
    raw_value = str(payload or "").strip()
    if not raw_value.startswith("data:") or ";base64," not in raw_value:
        raise HTTPException(status_code=400, detail="Uploaded file must be a valid base64 data URL")
    prefix, encoded_payload = raw_value.split(",", 1)
    media_type = prefix[5:].split(";", 1)[0].strip().lower()
    try:
        decoded = base64.b64decode(encoded_payload, validate=True)
    except (ValueError, binascii.Error) as exc:
        raise HTTPException(status_code=400, detail="Uploaded file could not be decoded") from exc
    return media_type, decoded


def validate_profile_photo_data(photo_data: Any) -> str:
    value = str(photo_data or "").strip()
    if not value:
        return ""
    media_type, file_bytes = decode_base64_data_url(value)
    if media_type not in ALLOWED_PROFILE_PHOTO_TYPES:
...
def validate_resume_payload(payload: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise HTTPException(status_code=400, detail="Resume payload is invalid")
    file_name = str(payload.get("fileName") or "").strip()
    if not file_name.lower().endswith(".pdf"):
        raise HTTPException(status_code=400, detail="Resume must be uploaded as a PDF file")
    file_data = str(payload.get("fileData") or "").strip()
    if not file_data:
        raise HTTPException(status_code=400, detail="Resume file data is required")
    media_type, file_bytes = decode_base64_data_url(file_data)
    ...
    if len(file_bytes) > MAX_RESUME_UPLOAD_BYTES:
        raise HTTPException(status_code=400, detail="Resume PDF must be 5 MB or smaller")
    if not file_bytes.startswith(b"%PDF-"):
        raise HTTPException(status_code=400, detail="Resume file does not appear to be a valid PDF")
    return {
        **payload,
        "fileName": file_name.replace('"', "").replace("\r", "").replace("\n", ""),
...
def decode_resume_payload(resume: dict[str, Any]) -> tuple[bytes, str, str]:
    validated_resume = validate_resume_payload(resume)
    media_type, file_bytes = decode_base64_data_url(str(validated_resume.get("fileData") or ""))
    safe_file_name = str(validated_resume.get("fileName") or "resume")
...
def start_interview_chat_session(journey: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "proctoring": {
            ...
            "behaviorMetrics": empty_interview_behavior_metrics(),
            "events": [],
        },
...
def empty_interview_behavior_metrics() -> dict[str, int]:
    return {
        "frameSamples": 0,
        "faceVisibleFrames": 0,
        "centeredFrames": 0,
...
def merge_interview_behavior_metrics(current: Any, incoming: Any) -> dict[str, int]:
    merged = empty_interview_behavior_metrics()
    if isinstance(current, dict):
        for key in merged:
            try:
                merged[key] = max(0, int(current.get(key, 0) or 0))
            except (TypeError, ValueError):
                merged[key] = 0
    if isinstance(incoming, dict):
        for key in merged:
            try:
                merged[key] += max(0, int(incoming.get(key, 0) or 0))
            except (TypeError, ValueError):
                continue
    return merged
...
def serialize_employee(record: EmployeeRecord) -> dict[str, Any]:
    return {
        ...
        "lastAssessment": record.last_assessment,
        "photoData": "",
    }
...
def validate_managed_employee_password(value: Any) -> str:
...
def normalize_employee_account_id(value: Any) -> str:
    raw_value = str(value or "").strip().upper()
    if not raw_value:
        return ""
    normalized = "".join(char for char in raw_value if char.isalnum() or char in {"-", "_"})
    if not normalized or len(normalized) < 3 or len(normalized) > 64:
        raise HTTPException(status_code=400, detail="Employee ID must be 3-64 characters using letters, numbers, dashes, or underscores")
    return normalized


def generate_employee_account_id(session) -> str:
    existing_ids = session.scalars(select(UserRecord.id).where(UserRecord.role == "employee")).all()
    max_number = 1000
    for user_id in existing_ids:
        raw_value = str(user_id or "").strip().upper()
        if raw_value.startswith("EMP-") and raw_value[4:].isdigit():
            max_number = max(max_number, int(raw_value[4:]))
    return f"EMP-{max_number + 1}"


def get_employee_account_by_email(session, email: str) -> UserRecord | None:
...
def get_employee_account_by_id(session, employee_id: str) -> UserRecord | None:
    normalized_id = normalize_employee_account_id(employee_id)
    if not normalized_id:
        return None
    return session.get(UserRecord, normalized_id)


def migrate_state_key(session, old_key: str, new_key: str, default_payload: Any) -> None:
    if old_key == new_key:
        return
    payload = get_state(session, old_key, default_payload)
    if payload not in ({}, "", None):
        set_state(session, new_key, payload)
    set_state(session, old_key, default_payload)


def migrate_employee_account_references(session, old_employee_id: str, new_employee_id: str) -> None:
    if old_employee_id == new_employee_id:
        return

    for record in session.scalars(select(AssessmentRecord).where(AssessmentRecord.employee_id == old_employee_id)).all():
        record.employee_id = new_employee_id
        profile_payload = loads_json(record.profile_json, {})
        if isinstance(profile_payload, dict) and str(profile_payload.get("employeeId") or "").strip() == old_employee_id:
            profile_payload["employeeId"] = new_employee_id
            record.profile_json = dumps_json(profile_payload)

    for record in session.scalars(select(AssessmentAttemptRecord).where(AssessmentAttemptRecord.employee_id == old_employee_id)).all():
        record.employee_id = new_employee_id

    for record in session.scalars(select(FeedbackSurveyRecord).where(FeedbackSurveyRecord.employee_id == old_employee_id)).all():
        record.employee_id = new_employee_id

    for record in session.scalars(select(ManagerInterviewScheduleRecord).where(ManagerInterviewScheduleRecord.employee_id == old_employee_id)).all():
        record.employee_id = new_employee_id

    for record in session.scalars(select(ManagerCommunicationRecord).where(ManagerCommunicationRecord.employee_id == old_employee_id)).all():
        record.employee_id = new_employee_id

    for record in session.scalars(select(NotificationRecord).where(NotificationRecord.user_id == old_employee_id)).all():
        record.user_id = new_employee_id

    journey = get_state(session, employee_journey_state_key(old_employee_id), {})
    if isinstance(journey, dict):
        profile = journey.get("profile", {}) if isinstance(journey.get("profile"), dict) else {}
        if str(profile.get("employeeId") or "").strip() == old_employee_id:
            profile["employeeId"] = new_employee_id
            journey["profile"] = profile
        set_state(session, employee_journey_state_key(new_employee_id), journey)
    set_state(session, employee_journey_state_key(old_employee_id), {})

    migrate_state_key(session, user_settings_state_key(old_employee_id), user_settings_state_key(new_employee_id), {})
    migrate_state_key(session, f"totp:{old_employee_id}", f"totp:{new_employee_id}", {"enabled": False, "secret": ""})
    migrate_state_key(session, f"weeklyReportNotice:{old_employee_id}", f"weeklyReportNotice:{new_employee_id}", "")
    migrate_state_key(session, f"weeklyReportEmail:{old_employee_id}", f"weeklyReportEmail:{new_employee_id}", "")

    legacy_journey = get_state(session, "employeeJourney", DEFAULT_EMPLOYEE_JOURNEY)
    if isinstance(legacy_journey, dict):
        legacy_profile = legacy_journey.get("profile", {}) if isinstance(legacy_journey.get("profile"), dict) else {}
        if str(legacy_profile.get("employeeId") or "").strip() == old_employee_id:
            legacy_profile["employeeId"] = new_employee_id
            legacy_journey["profile"] = legacy_profile
            set_state(session, "employeeJourney", legacy_journey)

    for token, details in list(ACTIVE_SESSIONS.items()):
        if details.get("userId") == old_employee_id:
            details["userId"] = new_employee_id

    for challenge in TWO_FACTOR_CHALLENGES.values():
        if challenge.get("userId") == old_employee_id:
            challenge["userId"] = new_employee_id

    for chat_state in INTERVIEW_CHAT_SESSIONS.values():
        if chat_state.get("userId") == old_employee_id:
            chat_state["userId"] = new_employee_id

    cache_invalidate(f"notifications:list:{old_employee_id}", f"notifications:list:{new_employee_id}")
...
def ensure_employee_account(
    ...
    *,
    ...
    account = get_employee_account_by_email(session, email)
    created = False
    department = role or "General"
    employee_id_was_provided = employee_id is not None
    requested_employee_id = normalize_employee_account_id(employee_id) if employee_id is not None else ""
    if account:
        previous_employee_id = str(account.id or "").strip()
        if employee_id_was_provided and not requested_employee_id:
            requested_employee_id = generate_employee_account_id(session)
        if requested_employee_id and requested_employee_id != previous_employee_id:
            conflicting_account = session.get(UserRecord, requested_employee_id)
            if conflicting_account:
                raise HTTPException(status_code=400, detail="Another employee already uses this employee ID")
            account.id = requested_employee_id
            migrate_employee_account_references(session, previous_employee_id, requested_employee_id)
        account.full_name = name or account.full_name
        account.email = email
        account.department = department
    else:
        final_employee_id = requested_employee_id or generate_employee_account_id(session)
        existing_id_owner = session.get(UserRecord, final_employee_id)
        ...
        account = UserRecord(
            ...
            email=email,
            role="employee",
            full_name=name or derive_name_from_email(email),
...
def build_employee_payloads(session, records: list[EmployeeRecord]) -> list[dict[str, Any]]:
    employee_users = session.scalars(select(UserRecord).where(UserRecord.role == "employee")).all()
    employee_by_email = {
        normalize_email(user.email): user
        for user in employee_users
        if normalize_email(user.email)
    }
    latest_assessments = session.execute(
        select(
        ...
        ).where(AssessmentRecord.employee_id.is_not(None)).order_by(AssessmentRecord.date.desc())
    ).all()
    latest_assessment_by_employee: dict[str, Any] = {}
    for row in latest_assessments:
        employee_id = str(row.employee_id or "").strip()
        if employee_id and employee_id not in latest_assessment_by_employee:
            latest_assessment_by_employee[employee_id] = row
    payload: list[dict[str, Any]] = []
    for record in records:
        entry = serialize_employee(record)
        account = employee_by_email.get(normalize_email(record.email))
        employee_id = str(account.id or "").strip() if account else ""
        journey = get_employee_journey_for_user(session, employee_id) if employee_id else {}
        profile = journey.get("profile", {}) if isinstance(journey.get("profile"), dict) else {}
        settings_profile = {}
        if employee_id:
            settings_payload = get_state(session, user_settings_state_key(employee_id), {})
            if isinstance(settings_payload, dict):
                settings_profile = (settings_payload.get("profile") if isinstance(settings_payload.get("profile"), dict) else {}) or {}
        latest_assessment = latest_assessment_by_employee.get(employee_id)
        assessment_profile = loads_json(latest_assessment.profile_json, {}) if latest_assessment and latest_assessment.profile_json else {}
        live_skills = [
            str(skill).strip()
            for skill in (
                (journey.get("skills", []) if isinstance(journey.get("skills"), list) else [])
                or loads_json(latest_assessment.selected_skills_json, []) if latest_assessment else []
                or entry.get("skills", [])
            )
            if str(skill).strip()
        ]
        live_name = (
            ...
            or (str(account.full_name or "").strip() if account else "")
            or str(assessment_profile.get("fullName") or "").strip()
            or entry["name"]
        )
        live_email = (
            str(profile.get("email") or "").strip()
            or (str(account.email or "").strip() if account else "")
            or entry["email"]
        )
        live_role = (
            str(profile.get("role") or "").strip()
            or str(assessment_profile.get("role") or "").strip()
            or entry["role"]
        )
        live_photo = (
            str(settings_profile.get("photoData") or "").strip()
            or str(profile.get("photoData") or "").strip()
            or str(assessment_profile.get("photoData") or "").strip()
        )
        if account and account.department and not profile.get("department"):
        ...
        entry.update(
            {
                ...
                "email": live_email,
                "role": live_role,
                "skills": list(dict.fromkeys(live_skills))[:8],
                "skillLevel": round(float(latest_assessment.score), 1) if latest_assessment and latest_assessment.score is not None else entry["skillLevel"],
                "status": profile_completion_status(profile, current_status=entry.get("status", "pending")),
                "photoData": live_photo,
            }
        )
...
def build_team_suggestions(session, team: TeamRecord) -> list[dict[str, Any]]:
    employees = build_employee_payloads(session, session.scalars(select(EmployeeRecord).order_by(EmployeeRecord.skill_level.desc())).all())
    if not employees:
        return []
...
def serialize_assessment(record: AssessmentRecord) -> dict[str, Any]:
    return {
        ...
        "behavioralSignals": loads_json(record.behavioral_signals_json, {}),
        "integritySignals": loads_json(record.integrity_signals_json, {}),
...
def build_assessment_pdf(payload: dict[str, Any]) -> bytes:
    ...
    behavioral = payload.get("behavioralSignals") if isinstance(payload.get("behavioralSignals"), dict) else {}
    integrity = payload.get("integritySignals") if isinstance(payload.get("integritySignals"), dict) else {}
    communication = behavioral.get("communication") if isinstance(behavioral.get("communication"), dict) else {}
    posture = behavioral.get("posture") if isinstance(behavioral.get("posture"), dict) else {}
    eye_movement = behavioral.get("eyeMovement") if isinstance(behavioral.get("eyeMovement"), dict) else {}
    movement = behavioral.get("movement") if isinstance(behavioral.get("movement"), dict) else {}

    y -= 8
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(72, y, "Behavioral Signals")
    y -= 16
    pdf.setFont("Helvetica", 10)
    for line in [
        f"Communication: {format_score_value(communication.get('score'))}/5 - {communication.get('label', 'Pending')}",
        f"Posture Confidence: {format_score_value(posture.get('score'))}/5 - {posture.get('confidence', 'Pending')}",
        f"Eye Movement Stability: {format_score_value(eye_movement.get('score'))}/5 - {eye_movement.get('label', 'Pending')}",
        f"Movement Stability: {format_score_value(movement.get('score'))}/5 - {movement.get('label', 'Pending')}",
        f"Integrity Risk: {integrity.get('cheatingRisk', 'Pending')}",
    ]:
        pdf.drawString(84, y, line)
        y -= 14
...
def build_dynamic_dashboard(session) -> dict[str, Any]:
    employees = build_employee_payloads(session, session.scalars(select(EmployeeRecord)).all())
...
def upsert_assessment(session, payload: dict[str, Any]) -> None:
    ...
    record.behavioral_signals_json = dumps_json(payload.get("behavioralSignals", {}))
    record.integrity_signals_json = dumps_json(payload.get("integritySignals", {}))
...
def build_manager_candidate_workbench(session) -> dict[str, Any]:
    ...
    for employee in employees:
        ...
        row = {
            ...
            "twoFactor": totp_status(session, employee.id),
            "contact": {
                "email": contact_email,
                "phone": contact_phone,
...
async def attach_session_cookie(request, call_next):
    ...
    if session_token:
        session_state = ACTIVE_SESSIONS.get(session_token)
        csrf_token = str((session_state or {}).get("csrfToken") or "").strip()
        if session_state and not csrf_token:
            csrf_token = secrets.token_urlsafe(24)
            session_state["csrfToken"] = csrf_token
        if csrf_token and str(request.cookies.get(CSRF_COOKIE_NAME) or "").strip() != csrf_token:
            response.set_cookie(
                key=CSRF_COOKIE_NAME,
                value=csrf_token,
                httponly=False,
...
def startup_event() -> None:
    ...
    with db_session() as session:
        ...
        employees_payload = build_employee_payloads(session, session.scalars(select(EmployeeRecord).order_by(EmployeeRecord.id)).all())
...
def manager_login(payload: dict[str, Any], response: Response, request: Request) -> dict[str, Any]:
    require_human_verification(payload)
    email = normalize_email(payload.get("email", ""))
    rate_limit_key = enforce_auth_rate_limit("manager_login", request, email)
    lockout_key = enforce_auth_lockout("manager_login", request, email)
    with db_session() as session:
        security = get_security_settings(session)
        user = session.scalar(select(UserRecord).where(func.lower(UserRecord.email) == email, UserRecord.role == "manager"))
        if not user or not verify_user_password(user, str(payload.get("password", ""))):
            register_auth_failure(
                ...
                actor_role="manager",
                actor_name="Manager",
                target=email or "unknown-manager",
            )
            raise HTTPException(status_code=401, detail="Invalid manager credentials")
        maybe_upgrade_password_hash(user, str(payload.get("password", "")))
        ...
        clear_auth_lockout(lockout_key)
        return issue_session_response(user, response)


@app.post("/api/auth/employee/login")
def employee_login(payload: dict[str, Any], response: Response, request: Request) -> dict[str, Any]:
    require_human_verification(payload)
    raw_email = str(payload.get("email", "")).strip()
    email = raw_email.lower()
    employee_id = str(payload.get("employeeId", "")).strip()
    rate_limit_identity = email or employee_id
    rate_limit_key = enforce_auth_rate_limit("employee_login", request, rate_limit_identity)
    lockout_key = enforce_auth_lockout("employee_login", request, rate_limit_identity)
    with db_session() as session:
        security = get_security_settings(session)
        ...
        if email:
            ...
            if user and employee_id and str(user.id).lower() != employee_id.lower():
                register_auth_failure(
                    ...
                    actor_role="employee",
                    actor_name="Employee",
                    target=email or employee_id or "unknown-employee",
                )
                raise HTTPException(
                    status_code=401,
        ...
        if not user or not verify_user_password(user, password_value):
            register_auth_failure(
                session,
                action="employee_login",
                request=request,
        ...
        maybe_upgrade_password_hash(user, password_value)
        ...
        if security.get("twoFactorRequired", True) and totp.get("enabled"):
            ...
            if not verify_totp_code(get_state(session, f"totp:{user.id}", {}).get("secret", ""), code):
                register_auth_failure(
                    ...
                    action="employee_login",
                    request=request,
                    identity=rate_limit_identity or user.id,
                    actor_role="employee",
                    actor_name=user.full_name,
                    target=user.email or user.id,
                )
                raise HTTPException(status_code=401, detail="Invalid two-factor code")
        append_audit_log(
            session,
...
def employee_register(payload: dict[str, Any], response: Response, request: Request) -> dict[str, Any]:
    require_human_verification(payload)
    email = normalize_email(payload.get("email"))
    enforce_auth_rate_limit("employee_register", request, email)
    with db_session() as session:
        security = get_security_settings(session)
        existing = session.scalar(select(UserRecord).where(func.lower(UserRecord.email) == email))
        if existing:
            raise HTTPException(status_code=400, detail="Email already registered")
        password_value = validate_managed_employee_password(payload.get("password", ""))
        user = UserRecord(id=payload["employeeId"], email=email, role="employee", full_name=payload["fullName"], department=payload["department"], password_hash=hash_password(password_value))
        session.add(user)
        session.flush()
...
def two_factor_setup(authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    return {"secret": secret, "otpauthUrl": otpauth_url, "qrCodeUrl": "", **state}
...
def change_password(payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    new_password = validate_managed_employee_password(payload.get("newPassword", ""))

    with db_session() as session:
        user_record = session.get(UserRecord, user.id)
        if not user_record or not verify_user_password(user_record, current_password):
...
def update_settings(section: str, payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    with db_session() as session:
        ...
        cache_invalidate(
            f"notifications:list:{user_record.id}",
            "employees:list",
            "dashboard:summary",
            "teams:list",
            "manager:workbench:*",
...
def update_manager_candidate_profile(employee_id: str, payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    with db_session() as session:
        ...
        sync_employee_record_from_profile(
            session,
            employee_id,
            normalized_payload,
            fallback_email=employee.email,
            fallback_name=employee.full_name,
            fallback_department=employee.department,
        )
        ...
        cache_invalidate("resumes:index", "employees:list", "assessments:list", "dashboard:summary", "reports:summary", "teams:list", "manager:workbench:*")
...
@app.put("/api/manager/candidates/{employee_id}/settings/{section}")
def update_manager_candidate_settings(
    employee_id: str,
    section: str,
    ...
    with db_session() as session:
        ...
        updated_settings = update_settings_for_user(session, employee, section, payload)
        append_audit_log(
            session,
            actor_id=user.id,
            actor_role=user.role,
            actor_name=user.full_name,
            action="manager_update_candidate_settings",
            target=employee.full_name or employee.email or employee_id,
            meta={"employeeId": employee_id, "section": section},
        )
        cache_invalidate("manager:workbench:*", f"notifications:list:{employee_id}")
        return {
            "employeeId": employee_id,
            "section": section,
            "settings": updated_settings,
            "twoFactor": totp_status(session, employee_id),
        }


@app.post("/api/manager/candidates/{employee_id}/2fa/enable")
def manager_enable_candidate_two_factor(employee_id: str, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    user = resolve_user_from_auth(authorization)
    if not user or user.role != "manager":
        raise HTTPException(status_code=401, detail="Unauthorized")
    ...
    with db_session() as session:
        ...
        current = get_state(session, f"totp:{employee_id}", {"enabled": False, "secret": ""})
        secret = str(current.get("secret") or "").strip() or generate_totp_secret()
        status_payload = set_totp_state(session, employee_id, secret, True)
        create_inapp_notification(
            session,
            employee,
            "Two-factor authentication enabled",
            "A manager enabled two-factor authentication for your account. Use your authenticator setup key from settings if needed.",
            "security",
        )
        append_audit_log(
            session,
            ...
            actor_role=user.role,
            actor_name=user.full_name,
            action="manager_enable_candidate_2fa",
            target=employee.full_name or employee.email or employee_id,
            meta={"employeeId": employee_id},
        )
        cache_invalidate("manager:workbench:*", f"notifications:list:{employee_id}")
        return {"employeeId": employee_id, "twoFactor": status_payload}


@app.post("/api/manager/candidates/{employee_id}/2fa/disable")
def manager_disable_candidate_two_factor(employee_id: str, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    user = resolve_user_from_auth(authorization)
    if not user or user.role != "manager":
        raise HTTPException(status_code=401, detail="Unauthorized")
    ...
    with db_session() as session:
        ...
        status_payload = set_totp_state(session, employee_id, "", False)
        create_inapp_notification(
            session,
            employee,
            "Two-factor authentication disabled",
            "A manager disabled two-factor authentication for your account.",
            "security",
        )
        append_audit_log(
            session,
            ...
            actor_role=user.role,
            actor_name=user.full_name,
            action="manager_disable_candidate_2fa",
            target=employee.full_name or employee.email or employee_id,
            meta={"employeeId": employee_id},
        )
        cache_invalidate("manager:workbench:*", f"notifications:list:{employee_id}")
...
def get_employees() -> list[dict[str, Any]]:
    ...
    if cached is not None:
        return cached
    with db_session() as session:
        payload = build_employee_payloads(session, session.scalars(select(EmployeeRecord).order_by(EmployeeRecord.id)).all())
...
def create_employee(payload: dict[str, Any]) -> list[dict[str, Any]]:
    with db_session() as session:
        ...
        existing_employee = session.scalar(
            select(EmployeeRecord).where(func.lower(EmployeeRecord.email) == email)
        )
        if existing_employee:
            raise HTTPException(status_code=400, detail="An employee with this email already exists")
        if get_employee_account_by_email(session, email):
            raise HTTPException(status_code=400, detail="An employee account with this email already exists")

        password = validate_managed_employee_password(payload.get("password", ""))
        account, _ = ensure_employee_account(
            session,
            name=name,
            email=email,
            role=role,
        ...
        session.add(
            EmployeeRecord(
                ...
                email=email,
                role=role,
                skills_json=dumps_json(payload.get("skills", [])),
                skill_level=float(payload.get("skillLevel", 0)),
                status=payload.get("status", "pending"),
                last_assessment=payload.get("lastAssessment", "Never"),
            )
        )
        append_audit_log(
            session,
            ...
            target=email,
            meta={"role": role, "employeeId": account.id, "credentialsCreated": True},
        )
...
def update_employee(employee_id: int, payload: dict[str, Any]) -> list[dict[str, Any]]:
    with db_session() as session:
        ...
        next_name = str(payload.get("name", item.name)).strip()
        next_email = normalize_email(payload.get("email", item.email))
        next_role = str(payload.get("role", item.role)).strip()
        if not next_name or not next_email or not next_role:
            raise HTTPException(status_code=400, detail="Name, email, and role are required")
        conflicting_employee = session.scalar(
            select(EmployeeRecord).where(
                func.lower(EmployeeRecord.email) == next_email,
        ...
        other_account = get_employee_account_by_email(session, next_email)
        original_email = normalize_email(item.email)
        if other_account and normalize_email(other_account.email) != original_email:
            raise HTTPException(status_code=400, detail="Another employee account already uses this email")

        password = payload.get("password")
        managed_password = None
        if password is not None and str(password).strip():
            managed_password = validate_managed_employee_password(password)

        account, account_created = ensure_employee_account(
            session,
            name=next_name,
            email=next_email,
            role=next_role,
        ...
        append_audit_log(
            ...
            meta={
                ...
                "passwordUpdated": bool(managed_password),
...
def get_roles() -> list[dict[str, Any]]:
    ...
    if cached is not None:
        return cached
    with db_session() as session:
        employees = build_employee_payloads(session, session.scalars(select(EmployeeRecord).order_by(EmployeeRecord.id)).all())
        payload = [
            serialize_role(
                item,
...
def get_teams(authorization: str | None = Header(default=None)) -> list[dict[str, Any]]:
    ...
    if cached is not None:
        return cached
    with db_session() as session:
        employees = build_employee_payloads(session, session.scalars(select(EmployeeRecord).order_by(EmployeeRecord.name)).all())
...
def save_employee_profile(payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    with db_session() as session:
        ...
        normalized_payload["photoData"] = validate_profile_photo_data(normalized_payload.get("photoData"))
        journey["profile"] = normalized_payload
        ...
        if employee_id:
            ...
            sync_employee_record_from_profile(
                ...
                employee_id,
                normalized_payload,
                fallback_email=user.email if user else email,
                fallback_name=user.full_name if user else full_name,
                fallback_department=user.department if user else department,
            )
        ...
        cache_invalidate("resumes:index", "employees:list", "dashboard:summary", "teams:list", "manager:workbench:*")
...
def save_employee_resume(payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, Any]:
    payload = validate_resume_payload(payload)
...
def report_employee_interview_proctoring(payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    if action not in {"warn", "clear", "cancel", "observe"}:
        action = "warn"
    ...
    with db_session() as session:
        ...
        now_iso = datetime.now(UTC).isoformat()
        behavior_metrics = merge_interview_behavior_metrics(proctoring.get("behaviorMetrics"), payload.get("metrics"))
        if event_type == "window_focus_lost":
            behavior_metrics["focusLossEvents"] += 1
        elif event_type == "tab_hidden":
            behavior_metrics["tabHiddenEvents"] += 1
        elif event_type == "no_face_detected":
            behavior_metrics["noFaceMoments"] += 1
        elif event_type in {"start_precheck_multi_person", "live_multi_person_detected"}:
            behavior_metrics["multiPersonMoments"] += 1
        ...
        events.append(
            {
                ...
                "createdAt": now_iso,
                "metrics": {
                    "focusLossEvents": behavior_metrics.get("focusLossEvents", 0),
                    "tabHiddenEvents": behavior_metrics.get("tabHiddenEvents", 0),
                    "multiPersonMoments": behavior_metrics.get("multiPersonMoments", 0),
                    "noFaceMoments": behavior_metrics.get("noFaceMoments", 0),
                    "gazeShiftMoments": behavior_metrics.get("gazeShiftMoments", 0),
                },
            }
        )
        events = events[-40:]
...
def complete_employee_interview(payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    with db_session() as session:
        ...
        behavior_metrics = merge_interview_behavior_metrics({}, payload.get("behaviorMetrics"))
        if session_id:
            chat_state = get_interview_chat_state(session, session_id)
            ...
            if chat_state:
                ...
                behavior_metrics = merge_interview_behavior_metrics(behavior_metrics, proctoring_state.get("behaviorMetrics"))
